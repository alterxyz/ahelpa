...
ahelpa captures Kimi's `session_*` ID after submitting the first task message; the ID does not exist during initial startup. When `[AHELPA:DONE]` appears, `resume` is rejected while the old helper is draining. Wait until `ahelpa check` reports `idle` and its terminal is gone, or use the explicit reclamation path:
...
The driver reconnects through `kimi --session <id>` in a new tmux session. It reuses an explicitly supplied launch-time model alias and the original safe posture; otherwise Kimi uses its configured default. Completed and legacy dead records with an `agentResumeId` remain resumable until `ahelpa clean`; cleaning it deletes the resume metadata. Persistent conversation does not mean the original tmux process remains alive forever.
